...
//  }
  ...
  /**
   * Test paragraphs which are nested inside other paragraphs.
   */
  public function testNestedParagraphs() {
    ...
    // Create a paragraph type with fields for text and a reference.
    $this->drupalPostForm('admin/structure/paragraphs_type/add', [
      'label' => 'Nested Paragraph',
      'id' => 'nested_prgph',
    ], t('Save'));
    $this->drupalPostForm('admin/structure/paragraphs_type/nested_prgph/fields/add-field', [
      'label' => 'Paragraph',
      'field_name' => 'prgph_text',
    ...
    $this->drupalPostForm('admin/structure/paragraphs_type/nested_prgph/form-display', [
      'fields[field_prgrph_nested][type]' => 'entity_reference_paragraphs'
    ], t('Save'));

    // Add the new paragraph type to the content type article.
    ...
    $this->drupalPostForm('admin/structure/types/manage/article/form-display', [
      'fields[field_field_paragraph][type]' => 'entity_reference_paragraphs'
    ], t('Save'));

    // Remove the body field from the content type article.
    $this->drupalPostForm('admin/structure/types/manage/article/fields/node.article.body/delete', [], t('Delete'));
    $this->drupalPostForm('node/add/article', [
      'title[0][value]' => 'Test article'
    ], t('Add Nested Paragraph'));
    $this->drupalPostForm(NULL, [
      'field_field_paragraph[0][subform][field_prgph_text][0][value]' => 'This'
    ], t('Add Nested Paragraph'));
    $this->drupalPostForm(NULL, [
      'field_field_paragraph[0][subform][field_prgrph_nested][0][subform][field_prgph_text][0][value]' => 'is'
    ], t('Add another item'));
    $this->drupalPostForm(NULL, [
      'field_field_paragraph[0][subform][field_prgrph_nested][0][subform][field_prgph_text][1][value]' => 'a'
    ], t('Add Nested Paragraph'));
    $this->drupalPostForm(NULL, [
      'field_field_paragraph[0][subform][field_prgrph_nested][1][subform][field_prgph_text][0][value]' => 'test'
    ], t('Save and publish'));
    $content = $this->xpath('//div[@class="field field--name-field-field-paragraph field--type-entity-reference-revisions field--label-above"]')[0]->div[1]->div;
    $first_paragraph_content = (string) $content->div->div[1]->div->p;
    $nested_paragraph_one = (string) $content->div[1]->div[1]->div->div->div->div[1]->div[0]->p;
    $nested_paragraph_two = (string) $content->div[1]->div[1]->div->div->div->div[1]->div[1]->p;
    $second_paragraph_content = (string) $content->div->div[1]->div->p;
    debug($content->div[1]);
  }
